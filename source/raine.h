...
{
   char exe_path[256]; 		// current path of raine.exe
   char share_path[256];		// location of data files
#ifndef SDL
   char language_file[256];		// language translation file (no path)
#endif
   char screen_dir[256];		// screenshot path
   char emudx_dir[256];		// screenshot path
   char artwork_dir[256];		// screenshot path
   char **rom_dir;		// rom paths
   char config_file[256];		// config file (no path)
