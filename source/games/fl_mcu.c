...
static void m68705int() {
    ...
    printf("68705 int at %x\n",m68705.pc);
    flstory_mcu(1);
    printf("68705 int finished : %x\n",m68705.pc);
    m68705 = backup;
}
...
//if (errorlog) fprintf(errorlog,"%04x: 68705 port B write %02x\n",cpu_get_pc(),data);
	...
	if ((ddrB & 0x02) && (~data & 0x02) && (portB_out & 0x02))
	{
		...
		main_sent = 0;
		printf("from_main lue:%x, main_set = 0\n",from_main);
...
// if (errorlog) fprintf(errorlog,"send command %02x to main cpu\n",portA_out);
		from_mcu = portA_out;
		mcu_sent = 1;
		printf("mcu_sent = 1, from_mcu = portA %x\n",from_mcu);
	}
...
// if (errorlog) fprintf(errorlog,"%04x: 68705 port C read %02x\n",cpu_get_pc(),portC_in);
	printf("68705: portC mcu_sent %d portC_in %x res %x\n",mcu_sent,portC_in,(portC_out & ddrC) | (portC_in & ~ddrC));
...
void flstory_mcu_w(UINT32 offset,UINT8 data)
{
	from_main = data;
	main_sent = 1;
	printf("*** main_sent = 1 from %x data %x\n",z80pc,data);
	if (*fl_mcu_started)
	    m68705int();
	else
	    *fl_mcu_started = 1;
