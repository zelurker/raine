...
int tms5220_sh_start(const struct TMS5220interface *msound)
{
    intf = msound;
	...
	/* initialize a stream */
	stream = stream_init("TMS5220", audio_sample_rate, 16, 0, tms5220_update);
	if (stream == -1)
		return 1;
	stream_set_volume(stream,intf->mixing_level);
...
void tms5220_set_frequency(int frequency)
{
	...
	if (stream != -1)
		stream_update(stream, 0);
	source_step = (UINT32)((double)(frequency / 80) * (double)FRAC_ONE / (double)audio_sample_rate);
}
