...
#define STEPS 16
#define LFO_VOLUME 6
#define SHOOT_VOLUME 40
#define NOISE_VOLUME 40
...
static int channellfo1,channellfo2;
...
void galaxian_noise_timer_cb(int param)
{
	if( noisevolume > 0 )
	{
		...
		set_sample_volume(channelnoise,noisevolume);
	}
}
...
{
	{
		...
		else
		{
		    ...
		    if (noisevolume == NOISE_VOLUME)
			noisetimer = timer_adjust( TIME_IN_USEC(0.693*(155000+22000)*22 / 100), 0, TIME_IN_USEC(0.693*(155000+22000)*22 / 100),galaxian_noise_timer_cb);
		}
	}
}
...
WRITE_HANDLER( galaxian_shoot_enable_w )
{
	...
	{
		{
			play_sample(channelshoot, shootwave, shoot_length, shoot_rate, 0, SHOOT_VOLUME);
		}
	}
	last_port2=data;
}
...
{
	int i, countdown, generator, bit1, bit2;

	if( (noisewave = AllocateMem(NOISE_LENGTH * sizeof(INT16))) == 0 )
	{
		return 1;
	}

#define SHOOT_SEC 2
	shoot_rate = audio_sample_rate;
	shoot_length = SHOOT_SEC * shoot_rate;
	if ((shootwave = AllocateMem(shoot_length * sizeof(INT16))) == 0)
		return 1;
...
#define NE555_FM_ADJUST_RATE 0.80
	...
	channelnoise = create_sample(noisewave,NOISE_LENGTH,NOISE_RATE,1,0);
	channelshoot = create_sample(shootwave,SHOOT_LENGTH,SHOOT_RATE,1,0);

	channellfo = create_sample(backgroundwave,sizeof(backgroundwave),1000,1,0);
	channellfo1 = create_sample(backgroundwave,sizeof(backgroundwave),1000,1,0);
	channellfo2 = create_sample(backgroundwave,sizeof(backgroundwave),1000,1,0);
...
{
	del_sample(channelnoise);
	del_sample(channelshoot);
	del_sample(channellfo);
	del_sample(channellfo1);
	del_sample(channellfo2);
}

WRITE_HANDLER( galaxian_background_enable_w )
{
    offset &= 3;
    // Actually these 3 lfo samples are playing at the same time with slightly different frequencies
    // it's better to set the volume very low !
    set_sample_volume(channellfo+offset,(data & 1) ? LFO_VOLUME : 0);
}
...
#undef Vcc
	...
	lfotimer = timer_adjust( TIME_IN_SEC(td / (MAXFREQ-MINFREQ)), 0, TIME_IN_SEC(td / (MAXFREQ-MINFREQ)),galaxian_lfo_timer_cb);
#else
	static int lfobit[4];
	...
	lfotimer = timer_adjust( TIME_IN_USEC(0.639 * rx / (MAXFREQ-MINFREQ)), 0, TIME_IN_USEC(0.639 * rx / (MAXFREQ-MINFREQ)),galaxian_lfo_timer_cb);
#endif
}
...
void galaxian_sh_update(void)
{
	...
	set_sample_frequency(channellfo+0, sizeof(backgroundwave)*freq*(100+2*470)/(100+2*470) );
	set_sample_frequency(channellfo+1, sizeof(backgroundwave)*freq*(100+2*330)/(100+2*470) );
	set_sample_frequency(channellfo+2, sizeof(backgroundwave)*freq*(100+2*220)/(100+2*470) );
}
