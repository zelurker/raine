...
#define PROCESS_POKEY(chip) 											\
	...
	if (pokey[chip].rtimer) timer_remove(pokey[chip].rtimer); \
        pokey[chip].rtimer = 0;
...
int pokey_sh_start(const struct POKEYinterface *msound)
{
    int chip;

    memcpy(&intf, msound, sizeof(struct POKEYinterface));
	...
	p->samplerate_24_8 = (audio_sample_rate) ? (intf.baseclock << 8) / audio_sample_rate : 1;
	p->divisor[CHAN1] = 4;
	p->divisor[CHAN2] = 4;
	p->divisor[CHAN3] = 4;
	p->divisor[CHAN4] = 4;
	...
	p->rtimer = NULL; // timer_alloc(NULL);

	p->timer[0] = NULL; // timer_alloc(pokey_timer_expire);
	p->timer[1] = NULL; // timer_alloc(pokey_timer_expire);
	p->timer[2] = NULL; // timer_alloc(pokey_timer_expire);

	p->ptimer[0] = NULL; // timer_alloc(pokey_pot_trigger);
	p->ptimer[1] = NULL; // timer_alloc(pokey_pot_trigger);
	p->ptimer[2] = NULL; // timer_alloc(pokey_pot_trigger);
	p->ptimer[3] = NULL; // timer_alloc(pokey_pot_trigger);
	p->ptimer[4] = NULL; // timer_alloc(pokey_pot_trigger);
	p->ptimer[5] = NULL; // timer_alloc(pokey_pot_trigger);
	p->ptimer[6] = NULL; // timer_alloc(pokey_pot_trigger);
	p->ptimer[7] = NULL; // timer_alloc(pokey_pot_trigger);
	...
	p->channel = stream_init(name, audio_sample_rate, 16, chip, update[chip]);
	...
	stream_set_volume(p->channel,intf.mixing_level[chip]);
    }
...
void pokey_timer_expire(int param)
{
	int chip = param >> 3;
	int timers = param & 7;
	struct POKEYregisters *p = &pokey[chip];
...
static void pokey_potgo(int chip)
{
    ...
    for( pot = 0; pot < 8; pot++ )
	{
		...
		p->ptimer[pot] = timer_adjust( TIME_IN_USEC(r * AD_TIME), (chip<<3)|pot, 0, pokey_pot_trigger);
			}
		}
	}
}
...
int pokey_register_r(int chip, int offs)
{
	...
	case POT4_C: case POT5_C: case POT6_C: case POT7_C:
		...
		if( p->pot_r[pot] )
		{
			...
			if( p->ALLPOT & (1 << pot) )
			{
				...
				printf("POKEY #%d read POT%d (interpolated) \n", chip, pot);
            }
			else
			{
				data = p->POTx[pot];
	...
	case RANDOM_C:
		...
		if( p->SKCTL & SK_RESET )
		{
			UINT32 adjust = (UINT32) (5/*(timer_timeelapsed(p->rtimer)*/ * intf.baseclock + 0.5);
		...
		if (p->rtimer) timer_remove(p->rtimer);
		p->rtimer = NULL;
		data = p->RANDOM ^ 0xff;
		break;
...
void pokey_register_w(int chip, int offs, int data)
{
	...
	if( chip >= intf.num )
	{
		printf("POKEY #%d is >= number of Pokeys!\n", chip);
		return;
	}
#endif
    ...
    case STIMER_C:
		...
		TIMER_DEL(p->timer[TIMER1]);
		TIMER_DEL(p->timer[TIMER2]);
		TIMER_DEL(p->timer[TIMER4]);
		...
		if( p->AUDCTL & CH12_JOINED )
        {
			if( p->divisor[CHAN2] > 4 )
			{
				...
				TIMER_DEL(p->timer[TIMER2]);
				p->timer[TIMER2] = timer_adjust( p->timer_period[TIMER2], p->timer_param[TIMER2], p->timer_period[TIMER2], pokey_timer_expire);
			}
        }
        else
        {
			if( p->divisor[CHAN1] > 4 )
			{
				...
				TIMER_DEL(p->timer[TIMER1]);
				p->timer[TIMER1] = timer_adjust( p->timer_period[TIMER1], p->timer_param[TIMER1], p->timer_period[TIMER1], pokey_timer_expire);
			}
		...
		if( p->AUDCTL & CH34_JOINED )
        {
			...
			if( p->AUDC[CHAN4] & 0xf0 )
            {
				if( p->divisor[CHAN4] > 4 )
				{
					...
					TIMER_DEL(p->timer[TIMER4]);
					p->timer[TIMER4] = timer_adjust( p->timer_period[TIMER4], p->timer_param[TIMER4], p->timer_period[TIMER4], pokey_timer_expire);
				}
            }
        }
        else
    ...
    if( ch_mask & (1 << CHAN1) )
    {
		...
		if( p->interrupt_cb  ) {
		    TIMER_DEL(p->timer[TIMER1]);
		    p->timer[TIMER1] = timer_adjust(1.0 * new_val / intf.baseclock, p->timer_param[TIMER1], p->timer_period[TIMER1], pokey_timer_expire);
		}
		p->audible[CHAN1] = !(
    ...
    if( ch_mask & (1 << CHAN2) )
    {
		...
		if( p->interrupt_cb ) {
		    TIMER_DEL(p->timer[TIMER2]);
		    p->timer[TIMER2] = timer_adjust( 1.0 * new_val / intf.baseclock, p->timer_param[TIMER2], p->timer_period[TIMER2], pokey_timer_expire);
		}
		p->audible[CHAN2] = !(
    ...
    if( ch_mask & (1 << CHAN4) )
    {
		...
		if( p->interrupt_cb) {
		    TIMER_DEL(p->timer[TIMER4]);
		    p->timer[TIMER4] = timer_adjust(1.0 * new_val / intf.baseclock, p->timer_param[TIMER4], p->timer_period[TIMER4], pokey_timer_expire);
		}
		p->audible[CHAN4] = !(
