...
static UINT32 ymf278_compute_decay_rate(int num)
{
	...
	return ((UINT64)samples * audio_sample_rate) / 44100;
}
...
#ifdef VERBOSE
			logerror("YMF278B: Decay step 1, dl=%d, val = %d rate = %d, delay = %g\n", slot->DL, slot->D1R, rate, ymf278_compute_decay_rate(rate)*1000.0/audio_sample_rate);
#endif
			if(rate<4)
				slot->env_vol_step = 0;
			else
...
#ifdef VERBOSE
		logerror("YMF278B: Decay step 2, val = %d, rate = %d, delay = %g, current vol = %d\n", slot->D2R, rate, ymf278_compute_decay_rate(rate)*1000.0/audio_sample_rate, slot->env_vol >> 23);
#endif
		if(rate<4)
			slot->env_vol_step = 0;
		else
...
#ifdef VERBOSE
		logerror("YMF278B: Release, val = %d, rate = %d, delay = %g\n", slot->RR, rate, ymf278_compute_decay_rate(rate)*1000.0/audio_sample_rate);
#endif
		if(rate<4)
			slot->env_vol_step = 0;
		else
...
static void ymf278b_timer_a_reset(int num)
{
  ...
  if(chip->enable & 1)
	...
	timer_remove(chip->timer_a);
      ...
      chip->timer_a = timer_adjust( TIME_IN_USEC(period), num, TIME_IN_USEC(period),ymf278b_timer_a_tick);
    }
...
static void ymf278b_timer_b_reset(int num)
{
  ...
  if(chip->enable & 2)
	...
	timer_remove(chip->timer_b);
      ...
      chip->timer_b = timer_adjust( TIME_IN_USEC(period), num, TIME_IN_USEC(period),ymf278b_timer_b_tick);
    }
		...
		switch((reg-8) / 24)
		{
			...
			case 4:
				...
				if (data & 0x80)
				{
					...
					slot->step = (((UINT64)step)*(44100/4)) / audio_sample_rate * chip->clock_ratio;
...
static void ymf278b_init(INT8 num, UINT8 *rom, void (*cb)(int), int clock)
{
	...
	YMF278B[num].rom = rom;
	YMF278B[num].timer_a = YMF278B[num].timer_b = NULL;
