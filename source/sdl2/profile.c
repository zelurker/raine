...
int use_rdtsc;
int profiler_mode = 0; // 0 rdtsc, 1 SDL
...
typedef struct PROFILE
{
   ...
   u64 start64, stop64; // same thing in 64 bits, planned for SDL_GetPerformanceCounter
...
void ProfileStart(UINT8 mode)
{
   ...
   if (!profiler_mode) {
       ...
       Profile[mode].start64 = SDL_GetPerformanceCounter();
...
void ProfileStop(UINT8 mode)
{
   ...
   if(Profile[mode].state==1){
	   ...
	   Profile[mode].stop64 = SDL_GetPerformanceCounter();
	   Profile[mode].cycles += (Profile[mode].stop64 - Profile[mode].start64);
       }
