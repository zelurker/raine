...
static int alt2_kludge(offs_t offset)
{
	...
	if (access_68k)
	{
...
#else
		// Starscream has no previous pc field for now, so it won't work
		// I'll put some default here, but it won't be good at all
		UINT32 pc = s68000_read_pc - 2;
#endif
		...
		if (MATCHES_MASK_VALUE((pc + 2) >> 1, slapstic.alt1))
		{
			...
			UINT16 opcode = ReadWord(&ROM[pc & 0xffffff]);
			if ((opcode & 0xf1f8) == 0x3090 || (opcode & 0xf1f8) == 0xb148)
			{
				...
				UINT32 regval = s68000_areg[((opcode >> 9) & 7)] >> 1;
...
static void slapstic_log(offs_t offset)
{
	...
	if (slapsticlog)
	{
		double time = emu_get_time();

		if (time - last_time > 1.0)
			fprintf(slapsticlog, "------------------------------------\n");
		last_time = time;

		fprintf(slapsticlog, "%06X: %04X B=%d ", s68000_read_pc-2, offset, current_bank);
		switch (state)
		{
			case DISABLED:
