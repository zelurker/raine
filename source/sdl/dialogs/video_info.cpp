...
TVideo_menu::TVideo_menu(char *my_title) : TMenu(my_title,NULL) {
  ...
  print_tf_state(2,"hardware surface available", video->hw_available);
  print_tf_state(3,"window manager available", video->wm_available);
  print_tf_state(4,"accelerated hardware->hardware blits", video->blit_hw);
  print_tf_state(5,"accelerated hardware->hardware colorkey blits", video->blit_hw_CC);
  print_tf_state(6,"accelerated hardware->hardware alpha blits", video->blit_hw_A);
  print_tf_state(7,"accelerated software->hardware blits", video->blit_sw);
  print_tf_state(8,"accelerated software->hardware colorkey blits", video->blit_sw_CC);
  print_tf_state(9,"accelerated software->hardware alpha blits", video->blit_sw_A);
  print_tf_state(10,"accelerated color fills", video->blit_fill);
